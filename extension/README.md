...
같은 패턴을 반복하면 된다. `lib/`에 수집 함수를 추가하고, `background.js`에 메시지 타입을 늘리고,
`page/`에 화면을 붙인다.

- 초과근무 분석 — `src/lib/timeinout.mjs`의 `fetchEmployeeCards` / `fetchEmployeeTrips` 대응.
  계산은 코어의 `cardsToByDay` + `buildDays`가 이미 한다
- 야근택시·야근식비 — 비즈플레이 도메인을 `host_permissions`에 추가해야 한다
- 상신·정정 신청 — 모달 조작과 파일 첨부가 얽혀 난이도가 가장 높다. 마지막에 옮긴다

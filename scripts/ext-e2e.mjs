...
// 실화면 변형: 실행 버튼 없이 파일 선택 즉시 업로드하고 iframe은 그대로 둔다.
// change 전에 감시하지 않거나 업로드 버튼을 필수로 보는 구현은 여기서 실패해야 한다.
const bzUploadInline = `<!doctype html><html><body><input type="file"><div id="status"></div>
<script>document.querySelector('input').onchange=async function(){var n=this.files[0].name;await fetch('/__expense_upload',{method:'POST'});document.getElementById('status').textContent='첨부 완료';var row=document.createElement('div');row.className='attached-file';row.textContent=n;parent.document.body.appendChild(row)}<\/script></body></html>`;
...
checks.push(['야근택시 수집·증빙 판정', yagunOk], ['야근식비 수집·인정 판정', yasikOk],
  ...
  ['결의서 하위 프레임의 파일선택 즉시 자동 업로드', inlineUploadLoads === 1],
